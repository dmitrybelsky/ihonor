...
### DB-ИНЪЕКЦИЯ — ПРОВЕРЕНА, НЕ ГОДИТСЯ
- Инжект note-строки (type=2, dirty=1, html/search_content) в локальную БД при закрытом app +
  релонч → заметка осталась `dirty=1, guid=""`, app НЕ зааплоадил. Реальные синкнутые заметки
  имеют `data1={"checkEdit":[],"checkRead":[]}` (у нашей NULL) + app триггерит upload не по
  note.dirty (in-app очередь/change-tracking, не DB-скан).
- ХУЖЕ: app **детектит изменение БД и ОТКЛЮЧАЕТ синхронизацию** (защита целостности). Восстановление
  из бэкапа БД вернуло синк. → DB-инъекция в живого клиента небезопасна, не годится.

### ПРАКТИЧЕСКИЕ ПУТИ WRITE (после проверок — все проблемны)
